...
# # 归一化小时和分钟标签
# hours = hours / 11.0  # 假设小时范围是 0-23
# minutes = minutes / 59.0  # 假设分钟范围是 0-59
...
# Split the labels into hours and minutes for training and test sets
y_train_hours = y_train[:, 0] / 11.0
y_train_minutes = y_train[:, 1] / 59.0
y_test_hours = y_test[:, 0] / 11.0
y_test_minutes = y_test[:, 1] / 59.0
...
for i, (image, hour, minute) in enumerate(zip(X_test[:5], predictions[0], predictions[1])):
    hour_value = int(hour[0] * 11)  # 反归一化小时
    minute_value = int(minute[0] * 59)  # 反归一化分钟
